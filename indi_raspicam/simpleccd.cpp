...
bool SimpleCCD::updateProperties()
{
    ...
    PrimaryCCD.setMinMaxStep("CCD_EXPOSURE", "CCD_EXPOSURE_VALUE", 0.0001, 5.9, 0.1, false);
...
void SimpleCCD::setupParams()
{
    ...
    //SetCCDParams(640, 480, 8, 5.4, 5.4);
    SetCCDParams(1296, 972, 8, 5.4, 5.4);
...
void SimpleCCD::grabImage()
{
    ...
    Mat img;
    img = imread("/run/shm/image.jpg", CV_LOAD_IMAGE_GRAYSCALE);
    for (int i = 0; i < height; i++)
        for (int j = 0; j < width; j++)
            {
              int y = PrimaryCCD.getSubY()+i;
              int x = PrimaryCCD.getSubX()+j;
              if ( x < PrimaryCCD.getXRes() && y < PrimaryCCD.getYRes())
                image[i * width + j] = (int)img.at<uchar>(y,x);
              else
                image[i * width + j] = 0;
            }
